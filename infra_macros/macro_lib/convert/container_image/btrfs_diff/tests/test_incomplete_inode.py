...
class IncompleteInodeTestCase(unittest.TestCase):
    ...
    def test_incomplete_file_including_common_attributes(self):
        ino = IncompleteFile(
            item=DumpItems.mkfile(path=b'a'), id_map=InodeIDMap(),
        )
        ...
        ino.apply_item(DumpItems.truncate(path=b'a', size=17))
        self.assertEqual('(IncompleteFile: a/17)', repr(ino))

        ino.apply_item(DumpItems.write(path=b'a', offset=10, len=15))
        self.assertEqual('(IncompleteFile: a/25)', repr(ino))

        ino.apply_item(
            DumpItems.update_extent(path=b'a', offset=40, len=5)
        )
        self.assertEqual('(IncompleteFile: a/45)', repr(ino))

        ino.apply_item(
            DumpItems.set_xattr(path=b'a', name=b'cat', data=b'nip')
        )
        self.assertEqual({b'cat': b'nip'}, ino.xattrs)

        ino.apply_item(DumpItems.remove_xattr(path=b'a', name=b'cat'))
        self.assertEqual({}, ino.xattrs)
        ...
        ino.apply_item(DumpItems.chmod(path=b'a', mode=0o4733))
        self.assertEqual(0o4733, ino.mode)

        with self.assertRaisesRegex(RuntimeError, 'cannot change file type'):
            ino.apply_item(DumpItems.chmod(path=b'a', mode=0o104733))

        ino.apply_item(DumpItems.chown(path=b'a', uid=1000, gid=2000))
        self.assertEqual(InodeOwner(uid=1000, gid=2000), ino.owner)

        ino.apply_item(
            DumpItems.utimes(path=b'a', ctime=1., mtime=2., atime=3.)
        )
        self.assertEqual(InodeUtimes(ctime=1., mtime=2., atime=3.), ino.utimes)

        class FakeItem(metaclass=DumpItem):
            pass
    ...
    def test_simple_file_types(self):
        for item_type, file_type, inode_type in (
            (DumpItems.mkdir, stat.S_IFDIR, IncompleteDir),
            (DumpItems.mkfifo, stat.S_IFIFO, IncompleteFifo),
            (DumpItems.mksock, stat.S_IFSOCK, IncompleteSocket),
        ):
    ...
    def test_devices(self):
        ino_chr = IncompleteDevice(
            item=DumpItems.mknod(path=b'chr', mode=0o20711, dev=123),
            id_map=InodeIDMap(),
        )
        ...
        ino_blk = IncompleteDevice(
            item=DumpItems.mknod(path=b'blk', mode=0o60544, dev=345),
            id_map=InodeIDMap(),
        )
        ...
        with self.assertRaisesRegex(RuntimeError, 'unexpected device mode'):
            IncompleteDevice(
                item=DumpItems.mknod(path=b'e', mode=0o10644, dev=3),
                id_map=InodeIDMap(),
            )

    def test_symlink(self):
        ino = IncompleteSymlink(
            item=DumpItems.symlink(path=b'l', dest=b'cat'),
            id_map=InodeIDMap(),
        )
        ...
        ino.apply_item(DumpItems.chown(path=b'l', uid=1, gid=2))
        ...
        with self.assertRaisesRegex(RuntimeError, 'cannot chmod symlink'):
            ino.apply_item(DumpItems.chmod(path=b'l', mode=0o644))
